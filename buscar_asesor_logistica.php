...
if (mysql_num_rows($result) >= 1) {
	...
	while ($row = mysql_fetch_array($result)) {
		...
		<option class='sugerencias_asesor_solicitante efecto-sugerencia' value='$row[idempleados];$row[nombre] $row[apellido_paterno] $row[apellido_materno];$colaborador'>$row[nombre] - $row[apellido_paterno] - $row[apellido_materno] $colaborador </option>
		</div>
		";
	}
}else{
	...
	if (mysql_num_rows($result1) >= 1) {
		...
		while ($row1 = mysql_fetch_array($result1)) {
			...
			<div class='content-op-busqueda-2'>
			<i class='fas fa-user icon-busqueda'></i>
			<option class='sugerencias_asesor_solicitante efecto-sugerencia' value='$row1[idcontacto];$row1[nombre] $row1[apellidos];$contacto'>$row1[nombre] - $row1[apellidos] - <b>$contacto</b> </option>
			</div>
			";
		}
		...
		if (mysql_num_rows($result2) >= 1) {
			...
			while ($row2 = mysql_fetch_array($result2)) {
				...
				<div class='content-op-busqueda-2'>
				<i class='fas fa-user icon-busqueda'></i>
				<option class='sugerencias_asesor_solicitante efecto-sugerencia' value='$row2[idproveedores];$row2[nombre] $row2[apellidos];$proveedor'>$row2[nombre] - $row2[apellidos] - <b>$proveedor</b> </option>
				</div>
				";
			}
			...
			if (mysql_num_rows($result3)) {
				...
				while ($row3 = mysql_fetch_array($result3)) {
					...
					<div class='content-op-busqueda-2'>
					<i class='fas fa-user icon-busqueda'></i>
					<option class='sugerencias_asesor_solicitante efecto-sugerencia' value='$row3[idorden_logistica_proveedores];$row3[nombre] $row3[apellidos];$proveedor_temporal'>$row3[nombre] - $row3[apellidos] - <b>$proveedor_temporal</b> </option>
					</div>
					";
				}
