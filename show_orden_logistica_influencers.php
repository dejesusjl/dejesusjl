...
$query_logisticas_sinotruck = "SELECT * FROM orden_logistica where columna_c = 'Influencers' and DATE_FORMAT(fecha_solicitud, '%Y-%m-%d') between '$fecha_a' and '$fecha_b'";
$result_logistica = mysql_query($query_logisticas_sinotruck);
...
#-------------------------------------------ID departamento--------------------------------------------------------------------------------
	$ver_departamento = name_departamento($row_logistica[iddepartamento]);

#-------------------------------------------Tipo orden--------------------------------------------------------------------------------
	$ver_torden = name_Torden($row_logistica[idcatalogo_orden_logistica]);

#-------------------------------------------Tipo orden--------------------------------------------------------------------------------
	$costo_logistica = costoTotalLogistica($row_logistica[idorden_logistica]);
...
#-------------------------------------------Trasladista--------------------------------------------------------------------------------
	...
	$nombre_asignante = $porciones_trasladista[2];

#-------------------------------------------Estatus Logisca--------------------------------------------------------------------------------

	$estatus_principal = estatusLogistica($row_logistica[idorden_logistica]);
	...
	$encriptado = base64_encode($row_logistica[idorden_logistica]);
	$link = "<a href='orden_logistica_detalles.php?idib=$encriptado' target='_blank'>$row_logistica[idorden_logistica]</a>";
	$mensaje .= "<tr>
	<td>$link</td>
...
function name_Torden($id_tipo_orden){
	...
	$query_tipo_orden = "SELECT * FROM orden_logistica_tipo_orden WHERE idorden_logistica_tipo_orden = '$id_tipo_orden'";
	$result_tipo_orden = mysql_query($query_tipo_orden);

	while ($row_tipo_orden = mysql_fetch_array($result_tipo_orden)) {
		$name_tipo_orden = $row_tipo_orden[nombre];
	}
...
function searchVIN($id_logistica_vin){
	...
	if (mysql_num_rows($result_busqueda_vin) == 0) {
		...
		while ($row_busqueda_vin = mysql_fetch_array($result_busqueda_vin)) {
			...
			$mostrar_unidad .= "<b>VIN:</b> $porciones_vin[0]<br>
