...
<head>
	...
	<title>Guardar colaborador Departamento</title>
...
<body>
				...
				<div class="d-flex justify-content-center">
					<div id="success_alert" class="neu-container-alert mt-4" style="display: none;">
						<div class="d-flex justify-content-center align-items-center neu-alert neu-alert-correcto">
							<h1 class="text-center text-white mb-0">Colaborador departamento agregado exitosamente</h1>
						</div>
					</div>
				</div>
	...
	$array_idempleado = $_POST['idempleado']; 
	$array_departamento = $_POST['iddepartamento']; 
	$fecha_creacion = $_POST['fecha_creacion']; 



	for ($i=0; $i < count($array_idempleado); $i++) { 

		if ($array_idempleado[$i] != "") {

			$idempleado = trim($array_idempleado[$i]);
			$iddepartamento = trim($array_departamento[$i]);

			$query_repetido = "SELECT * FROM orden_logistica_colaborador_departamento where trim(idempleado) = '$idempleado' and trim(iddepartamento) = '$iddepartamento' ";
			$result_repetido = mysql_query($query_repetido);

			if (mysql_num_rows($result_repetido) == 0) {

				$query_insert = "INSERT INTO orden_logistica_colaborador_departamento (idempleado, iddepartamento, visible, usuario_creador, fecha_creacion, fecha_guardado) values ('$idempleado', '$iddepartamento', 'SI', '$usuario_creador', '$fecha_creacion', '$fecha_guardado')";
				...
				if ($result_insert == 1) {
					...
					$ver_errores .= "- Error al guardar <b>$idempleado, $iddepartamento</b> <br>";
				}

			}else{

				$ver_errores .= "- Error al guardar <b>$idempleado, $iddepartamento</b> ya se encuentra registrado<br>";
	...
	<script>
		...
		$(document).ready(function(){
			$("#continuar").click(function(){
				location.replace(`catalogo_colaborador_departamento.php`);
			}); 
