...
if (mysql_num_rows($result) >= 1) {
	...
	while ($row = mysql_fetch_array($result)) {
		...
		<option class='sugerencias_id_vin_logisticaaaaaaa efecto-sugerencia' value='$row[idempleados];$row[columna_b];$colaborador'>$row[nombre] - $row[apellido_paterno] - $row[apellido_materno] $colaborador</option>
		</div>
		";
	}
}else{
	...
	if (mysql_num_rows($result1) >= 1) {
		...
		while ($row1 = mysql_fetch_array($result1)) {
			...
			$mensaje.="
			<div class='content-op-busqueda-2'>
			<option class='sugerencias_id_vin_logisticaaaaaaa efecto-sugerencia' value='$row1[idcontacto];$row1[nombre] $row1[apellidos];$contacto'>$row1[nombre] - $row1[apellidos] - $contacto</option>
			</div>
			";
		}
		...
		if (mysql_num_rows($result2) >= 1) {
			...
			while ($row2 = mysql_fetch_array($result2)) {
				...
				$mensaje.="
				<div class='content-op-busqueda-2'>
				<option class='sugerencias_id_vin_logisticaaaaaaa efecto-sugerencia' value='$row2[idproveedores];$row2[nombre] $row2[apellidos];$proveedor'>$row2[nombre] - $row2[apellidos] - $proveedor</option>
				</div>
				";
			}
			...
			if (mysql_num_rows($result3)) {
				...
				while ($row3 = mysql_fetch_array($result3)) {
					...
					$mensaje.="
					<div class='content-op-busqueda-2'>
					<option class='sugerencias_id_vin_logisticaaaaaaa efecto-sugerencia' value='$row3[idorden_logistica_proveedores];$row3[nombre] $row3[apellidos];$proveedor_temporal'>$row3[nombre] - $row3[apellidos] - $proveedor_temporal</option>
					</div>
					";
